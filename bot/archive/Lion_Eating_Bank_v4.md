...
> 손으로 만든 파일이 아니라 게임 저장소(`jimin326/jimin_pika`) `tools/build_v4.mjs`가 원본에서 조립한 파일이다. 고칠 일이 있으면 생성기나 원본을 고치고 다시 만든다.
> 모든 수치는 2026-09-02 현재 물리(공 vy ±40 클램프)에서 잰 것. 상세 근거·검증 절차는 `jimin_pika/tools/LION_MERGE_PLAN.md`.
...
| 단위 (`tools/v4_checks.mjs`, 합성 스냅샷) | 17/17: 틱그룹 가드, 데드볼 가드(15점제 포함), 스킬→sanitize→sync 순서(400틱 불일치 0), 깨진 스냅샷 5종 무예외, 무작위 3000 스냅샷 계약, 궤적 이탈 포기, 캠퍼 포기 |
| 회귀 (`tools/lion_bench.mjs`, 10시드) | 위 1절 표. 내서브 전 상대 100% |
...
## 6. 다시 만들기·벤치 (게임 저장소 `jimin_pika`의 `tools/`에서)

```
node --no-warnings build_v4.mjs                                   # AdaptiveCounter_v5_2.js + Lion_Eating_Bank_v3.js(TH_SEQS) + thunder_expect.json → ../bot/Lion_Eating_Bank_v4.js
node --no-warnings v4_checks.mjs                                  # 단위 17개
node --no-warnings lion_bench.mjs ../bot/Lion_Eating_Bank_v4.js ../bot/AdaptiveCounter_v5_2.js   # 10시드 벤치(상대는 파일 또는 builtin)
node --no-warnings lion_bench.mjs ../bot/Lion_Eating_Bank_v4.js builtin 101,202,303,404,505,606,707,808,909,1010 2 2   # 지연 2프레임
node --no-warnings lion_trace_serve.mjs ../bot/Lion_Eating_Bank_v4.js ../bot/Sajamokneun_v3_2.js  # 상대가 건드린 서브 랠리 추적
```
