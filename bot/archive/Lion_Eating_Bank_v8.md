...
생성기·검증 도구는 개발 저장소 jimin_pika 의 `tools/` 에 있다(이 저장소에는 봇 파일과 이 문서만 올린다).
`tools/v8/build_v8.mjs` 가 v7 에서 생성한다(**파일을 손으로 고치지 말고 생성기를 고칠 것**, 당일 노브만 예외).

```
node --no-warnings tools/v8/build_v8.mjs            # 기본 = STEP 5, kg/REACT 3, canon, fast, path, THUNDER 1
V8_THUNDER=0 node --no-warnings tools/v8/build_v8.mjs 5 out.js   # 썬더 끈 빌드
node --no-warnings tools/v4_checks.mjs bot/Lion_Eating_Bank_v8.js   # 17/17
```
...
24시드 × 좌우 = 48경기/칸, 지연 1, 10점 (`tools/rally_escape/cmp_bench.mjs`). 경기 W-L / 리시브 랠리 승률.
...
Chrome(headless, fast 30fps, 10점, 실제 Web Worker): AC·NetCamper·RedTeam 좌우 6전 6승(RT-v8R 2:10, 나머지 10:0), 썬더 26회 발동·포기 0, PAGEERROR 0 — `tools/v8/browser_v8_final_canon.json`.
...
## 6. 도구
- 생성기 `tools/v8/build_v8.mjs` (env: V8_PRED, V8_REACT, V8_STANDBY, V8_PRIO, V8_TOUCH, V8_PASS_REACH, V8_PASS_WINDOW, V8_THUNDER)
- `tools/v4_checks.mjs` 단위 검사 17개(썬더 꺼진 빌드는 5개 FAIL 이 정상)
- `tools/possession_audit/gate_verify.mjs [N] [bot]` 킬 게이트 ↔ 정확 모델 대조
- `tools/v8/recv_kinds.mjs` 리시브 랠리의 계획 종류·점프 출처(승패별), `tools/v8/trace_loss.mjs` 패 랠리 마지막 N틱, `tools/v8/touch_audit.mjs` 터치 감지 ↔ 엔진 기록
- `tools/rally_escape/cmp_bench.mjs`, `loss_diag.mjs` (경로 인자 가능)
- Chrome: `HARNESS_CONFIG="$(cat tools/v8/browser_cfg_v8.json)" node tools/harness/run.mjs` (스크래치 dist + serve_static 8767)
