...
`tools/dayof/diff_engine.mjs` 결과: 물리 상수·틱 규약 전부 동일, physics.js 는 주석만 변경. 바뀐 것은 스킬 층(`skill/`)과 스냅샷·응답 필드뿐.
...
시뮬 훅 `tools/skills/claw_real.mjs` 는 가짜 규칙(today.mjs)이 아니라 새 레포의 `skill/gauge.js`·`skill/claw.js` 를 그대로 붙여 실게임 배선 순서(gauge.observe → 시전 → claw.observe)로 돈다.
...
- Chrome 실기(새 레포 빌드, 헤드리스 Chrome, 실제 Worker·스킬 층, v14 vs skill-example_v1 좌우 5점): **5:0 / 0:5**, timeouts 0, invalid 0, >120ms 0, p99 ≈14ms. F12 에 `스킬 발동 tick=651 skillX=133 gauge=60` 뒤 `self.claw 첫 non-null {centerX:133, framesUntilStrike:23}` — 반환 `skillX` 가 실제 엔진에서 시전으로 등록됨을 확인. 결과 `tools/dayof/out/chrome_2026-09-05T03-49-30.json`.
- 한계: 시드 3개. 다른 팀 봇의 시전 습관은 모른다. 시전 타이밍 창(18~40)은 첫 값이며 실전 로그로 조정할 것.

## 당일 사용

```
ENGINE_ROOT=<새레포> node --no-warnings tools/dayof/gates.mjs bot/Lion_Eating_Bank_v14.js --base Lion_Eating_Bank_v13
ENGINE_ROOT=<새레포> node --no-warnings tools/eval_skill_real.mjs ./skills/claw_real.mjs Lion_Eating_Bank_v14 skill-example_v1 3
node tools/dayof/harness_dayof.mjs <새레포> --bot Lion_Eating_Bank_v14.js --opp skill-example_v1.js
```
