/* gates.mjs — 제출 게이트 일괄 실행(계획 §5). 사용: node --no-warnings bot-dev/dayof/gates.mjs <후보.js> [--base Lion_Eating_Bank_v12] [--opps OurBot_v12,NetCamper_v2] [--seeds 1] [--skip shadow,sk,rule]
 *   1 정적: 크기 ≤ 4MB · 최상위 decide · 로드 · SK 노브 값 출력
 *   2 shadow_diff: 후보 vs 기준의 x/y/hit 동일성(SK.on=false 또는 fire 만 켠 상태면 0 이어야 함. guard/resync 를 켰으면 정보용)
...
if (!SKIP.includes('shadow')) {
  ...
  const guardOn = decide && decide.__sk && decide.__sk.on && (decide.__sk.guard || decide.__sk.resync);
  R('shadow_diff x/y/hit 동일 vs ' + BASE, !!m && (Number(m[2]) === 0 || !!guardOn), m ? `${m[1]}틱 불일치 ${m[2]}${guardOn ? ' (guard/resync 켜져 있어 정보용)' : ''}` : '실행 실패\n' + tail(r.out, 5));
}
/* 3 sk_v2_test */
if (!SKIP.includes('sk')) {
...
if (!SKIP.includes('rule')) {
  ...
  const ok = r.code === 0 && /throws 0 invalid 0/.test(r.out) && />120ms 0/.test(r.out) && !/FAIL|NG\b/.test(r.out);
  const line = (r.out.match(/decide: .*$/m) || [''])[0], ret = (r.out.match(/returns: .*$/m) || [''])[0];
  R('rule_check', ok, (line + ' | ' + ret).trim() || '\n' + tail(r.out, 6));
}
