...
생성: `V8_THUNDER=1 V8_NET_HAZ=0 V8_DEF_HOLD_K=0 V8_TOUCH_MARGIN_EXACT=9 V8_NET_TOP=0 node --no-warnings bot-dev/v8/build_v8.mjs 32 src/code-here/Lion_Eating_Bank_v11.js`
...
## 벤치 (bot-dev/rally_escape/cmp_bench.mjs, 24시드×좌우=48경기/칸, 지연 1)
...
## Chrome 실기 2차 — 최종 ADAPT 2 (결과 bot-dev/ph0_recv/out/chrome_v11_adapt2_2026-09-04.json)
...
`node bot-dev/flyticks/apply_fly3.mjs src/code-here/Lion_Eating_Bank_v11.js` — findKillJump 체공 방향 전환 후보 [0,1,2]→[0,1,2,3] (v10_1 에 있던 것과 같은 패치, 앵커 2개·중복 적용 방지). 상한 3 = KILL_MAX_CONTACT 16 에서 유도, 4 이상은 동일. 근거 bot-dev/flyticks/FINDINGS.md.
...
Chrome 실기 (bot-dev/harness/run.mjs, fast, 10점, 스크래치 dist, 8분 상한, timing 훅):
