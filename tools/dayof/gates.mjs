/* gates.mjs — 제출 게이트 일괄 실행(계획 §5). 사용: node --no-warnings tools/dayof/gates.mjs <후보.js> [--base Lion_Eating_Bank_v12] [--opps OurBot_v12,NetCamper_v2] [--seeds 1] [--allow-shadow-diff "사유"] [--skip shadow,sk,rule]
...
const candPath = fs.existsSync(cand) ? path.resolve(cand) : path.join(ROOT, 'bot', cand.endsWith('.js') ? cand : cand + '.js');
...
if (!SKIP.includes('shadow')) {
  const r = run('tools/v8/shadow_diff.mjs', [BASE, candPath, OPPS, SEEDS]);
...
if (!SKIP.includes('sk')) {
  if (decide && decide.__sk && decide.__skState) {
    const r = run('tools/sk_v2_test.mjs', [BASE, candPath, OPPS, SEEDS]);
...
if (!SKIP.includes('rule')) {
  const r = run('tools/rule_check.mjs', [candPath]);
