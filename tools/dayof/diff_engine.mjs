 ...
 * 사용: node tools/dayof/diff_engine.mjs <새레포 루트 또는 그 src> [--full] [--ours <우리 레포 루트>]
 ...
 * 결과는 화면 + tools/dayof/out/diff_<시각>.txt */
import fs from 'node:fs'; import path from 'node:path'; import { spawnSync } from 'node:child_process'; import { fileURLToPath } from 'node:url';
const HERE = path.dirname(fileURLToPath(import.meta.url));
const args = process.argv.slice(2);
const opt = (k) => { const i = args.indexOf(k); return i >= 0 ? args[i + 1] : null; };
const FULL = args.includes('--full');
const OURS = path.resolve(opt('--ours') || process.env.OURS_ENGINE || path.join(HERE, '..', '..'));   // main 에는 엔진이 없다: --ours <이전 엔진 루트> 또는 OURS_ENGINE
const target = args.find((a) => !a.startsWith('--') && a !== opt('--ours'));
if (!target) { console.error('usage: node tools/dayof/diff_engine.mjs <새레포 루트 또는 src> [--full]'); process.exit(2); }
