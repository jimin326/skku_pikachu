 ...
 * 사용: node --no-warnings tools/eval_skill_real.mjs [skill=./skills/today.mjs] [bot=Lion_Eating_Bank_v12_1] [opp=OurBot_v12|builtin|경로] [NSEED=2] [--sk k=v,...] [--both]
...
const { RealGame, BotInput, setCustomRng, ENGINE_ROOT } = await import(pathToFileURL(path.join(ROOT, 'tools/sim_real.mjs')));
...
const skillMod = await import(pathToFileURL(path.resolve(ROOT, 'tools', skillPath)).href);
