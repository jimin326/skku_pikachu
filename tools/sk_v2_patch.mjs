 ...
 * 사용: node tools/sk_v2_patch.mjs [<in.js>] [<out.js>] [--check]      예: node tools/sk_v2_patch.mjs bot/Lion_Eating_Bank_v13.js bot/Lion_Eating_Bank_v13_1.js
 *   기본 in  = bot/Lion_Eating_Bank_v12.js
 *   기본 out = bot/Lion_Eating_Bank_v12_1.js
 *   --check: 앵커 존재 여부만 출력. 앵커가 정확히 1개씩이 아니면 아무것도 쓰지 않고 코드 1.
 * 채택 조건: SK.on=false 에서 v12 와 출력 동일 (node --no-warnings tools/v8/shadow_diff.mjs Lion_Eating_Bank_v12 Lion_Eating_Bank_v12_1 ...)
 * 기능 검증: node --no-warnings tools/sk_v2_test.mjs
 * 패치 7건:
 *   A0 머리말                — 파일명·문서 참조
...
const inF = pos[0] || 'bot/Lion_Eating_Bank_v12.js';
const outF = pos[1] || 'bot/Lion_Eating_Bank_v12_1.js';
...
const headNew = `/* Lion_Eating_Bank_v${outVer}.js — 사자먹는은행 제출 봇. v${inVer} + 스킬 어댑터 v2 (SK.on=false 면 v${inVer} 와 출력 동일. tools/sk_v2_patch.mjs 가 v${inVer} 에서 생성, 손으로 고치지 말 것).\n *   어댑터 v2 노브·검증: bot/Lion_Eating_Bank_v12_1.md. 그 외 설계·근거·벤치·당일 절차: Lion_Eating_Bank_v${inVer}.md (§번호는 이 파일의 주석과 대응)`;
