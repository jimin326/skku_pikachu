...
var MixedRoute = (function () {
  ...
  function copy(n){return {x:n.x,y:n.y,vy:n.vy,state:n.state,frame:n.frame,delay:n.delay,
    ...
    dead:n.dead,exit:n.exit,uncertain:n.uncertain,hitX:n.hitX,hitY:n.hitY,hitFrame:n.hitFrame,
  ...
  function world(n){
    ...
    if(Math.abs(n.bx-NET)<NH && n.by>NT){
  ...
  function tick(n,a){
    ...
    if(overlap && !n.collision){
      ...
      if(n.state===2){
        ...
        n.down=a.y===1;n.hitX=n.bx;n.hitY=n.by;n.hitFrame=n.f;
        if(a.y<0)n.setups++;
      }else {n.down=false;if(n.bvy<0)n.setups++;}
  ...
  function finish(n,oppX){
    if(!n.power || !n.down)return -Infinity;
    var b=copy(n), f=0, window=0;
    b.exit=false;
    ...
    // Favor a steep finishing contact close to the net and a short receiving
    // window. All distances are relative to CURRENT ball/opponent/geometry.
    var steep=n.power&&n.down;
    return (steep?100000:10000) + (steep?n.hitY*20-Math.abs(NET-n.hitX)*60:0)
      -window*300-f*50-n.f*5 + Math.abs(b.bx-oppX);
  }
  ...
  function step(s,base){
    ...
    // Grant a rally route ONLY AFTER GD has physically received the ball.
    // A changed upward, non-power ball overlapping our grounded player is
    // evidence of a self contact, not a prediction that receiving will succeed.
    if(!memory.granted && memory.previous && s.self.state===0 &&
        n.bx<NET && !n.power && n.bvy<0 && n.collision){
      var prior=root(memory.previous), predictedBall=copy(prior);
      for(var df=0;df<s.tick-memory.previous.tick;df++)world(predictedBall);
      if(predictedBall.bvx!==n.bvx || predictedBall.bvy!==n.bvy){
        memory.granted=true;memory.path=[];memory.expected=null;n.touches=1;
        audit.receiveStarts++;
      }
    }
    ...
    if(memory.granted && memory.expected && !matches(n,memory.expected)){
      ...
      var confirmedSelf=n.collision && !n.power && n.bvy<0;
  ...
  return {step:step,stats:function(){return audit;},model:{tick:tick,root:root},
