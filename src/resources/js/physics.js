...
/** @constant @type {number} maximum absolute vertical velocity of the ball */
 ...
 * We made the ball x coord range symmetric, but by widening it to [0, 432] rather than narrowing it
 * to [20, 412], so the non-terminating case described above is NOT the one we are in. Measured over
 * 1800 rallies x 3 seeds, this limit is never reached in either prediction function (0 / ~120k calls
 * to {@link expectedLandingPointXWhenPowerHit}, 0 / ~2.3M to {@link calculateExpectedLandingPointXFor}).
 * The [20, 412] variant, by contrast, hit it on 1.7-2.4% of the power-hit prediction calls. Avoiding
 * that was one of the reasons for choosing [0, 432]. This limit therefore stays purely defensive.
...
function processCollisionBetweenBallAndWorldAndSetBallPosition(ball) {
  ...
  // Keep vertical movement bounded so the ball cannot skip through the net
  // between frames at extreme velocities.
  ...
  /*
    ...
    Two ways to make it symmetric, both noted by the author of this reverse-engineered port:
    narrow the right to (GROUND_WIDTH - BALL_RADIUS), giving a ball-center range of [20, 412], or
    widen the left to 0, giving [0, 432]. We use the latter (team lead's call): both walls now bounce
    when the ball's *center* reaches the screen edge, so the ball goes half off-screen on both sides
    rather than on neither.

    Trade-off we accepted, measured over 1800 rallies x 3 seeds: the court geometry is symmetric
    (off-screen frames 3.74% left vs 3.71% right) but the left side still scores only 42.6-46.3%,
    a ~5pp bias whose cause we could not pin down. The [20, 412] variant measured 50.0-51.6%.
    In exchange, this variant never hits INFINITE_LOOP_LIMIT (the [20, 412] one does, on 1.7-2.4% of
    the calls to expectedLandingPointXWhenPowerHit), and rallies run shorter (607-641 frames vs
    888-912). See the ADR for the full comparison.

    All three copies of this condition must always be changed together, otherwise the landing point
    predictions would disagree with the physics they are predicting.
  */
  if (futureBallX < 0 || futureBallX > GROUND_WIDTH) {
...
function calculateExpectedLandingPointXFor(ball) {
  ...
  while (true) {
    ...
    // Keep the landing prediction in sync with the actual ball physics.
...
function expectedLandingPointXWhenPowerHit(
  ...
  while (true) {
    ...
    // Keep the power-hit prediction in sync with the actual ball physics.
    if (copyBall.yVelocity > BALL_MAX_Y_VELOCITY) {
      copyBall.yVelocity = BALL_MAX_Y_VELOCITY;
    } else if (copyBall.yVelocity < -BALL_MAX_Y_VELOCITY) {
      copyBall.yVelocity = -BALL_MAX_Y_VELOCITY;
