...
export function setUpUI(pikaVolley, ticker) {
  ...
  // One-time reset of the saved winning score (2026-08-28).
  //
  // The tournament default moved 15 -> 10, but `loadOptions` gives a saved
  // `pv-offline-winningScore` precedence over the default, so every machine
  // that had ever opened the game would have kept playing to 15 -- the
  // tournament PCs included, since they are exactly the machines the game has
  // been opened on most.
  //
  // Clearing the key rather than writing '10' means these machines follow
  // whatever the default is, now and the next time it changes. The marker
  // makes this fire once per browser: after it, a player's own choice is
  // saved and never touched again. Bump the suffix to force another reset.
  const WINNING_SCORE_RESET_KEY = 'pv-offline-winningScore-reset-1';
  if (localStorageWrapper.get(WINNING_SCORE_RESET_KEY) === null) {
    localStorageWrapper.remove('pv-offline-winningScore');
    localStorageWrapper.set(WINNING_SCORE_RESET_KEY, 'done');
  }
