'use strict';
/* AdaptiveCounter_v5_3
 * v5_2와 로직은 동일하고 상수 두 개만 바꿨다. 근거는 y속도 상한 40이 들어간
 * 수정 엔진(origin/main 1f3cecb) 기준 헤드리스 실측, 12시드 × 좌우 = 상대당 24경기:
 *
 *   ADAPT_CFG.MAX_BLEND  0.62 -> 0.30   적응 수비를 약하게. 상대의 최근 코스로
 *       수비 위치를 끌어당기는 힘을 줄인다. 강한 상대일수록 코스를 섞기 때문에
 *       학습값을 세게 믿으면 오히려 빈 곳이 생긴다.
 *   CFG.AIR_MAX          16 -> 22       공중에서 접촉을 노릴 수 있는 프레임 창을
 *       넓힌다. 점프 체공이 32프레임이라 16은 상승 구간만 쓰는 셈이었다.
 *
 * 6개 상대 평균 경기승률 53.5% -> 59.7%. 가장 크게 바뀐 것:
 *   vs Lion_Eating_Bank_v2   1-23 -> 11-13
 *   vs Lion_Eating_Bank_v1   3-21 ->  5-19
 *   vs Adaptive_v5          15- 9 -> 21- 3
 * 주의(비추이성): v5_2 원본과 붙으면 12-12 -> 6-18 로 진다. 리그로 판단할 것. */
