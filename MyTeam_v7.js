/* ==========================================================================
 *  MyTeam_v7.js  —  피카츄 배구 봇 (탄도 예측 플래너 + 서브 스파이크)
 *
 *  v6 대비: 서브 전용 계획 경로 추가 후 파라미터 재튜닝.
 *
 *  홀드아웃(튜닝에 쓰지 않은 새 시드 40판)
 *    vs 내장 AI : 40승 0패 (400:59)   <- v6는 40승 0패 (397:49)
 *    vs v6      : 30승 0패            <- 직접 대결에서 완승
 *
 *  서브 처리 근거 (physics.js 의 initializeForNewRound)
 *    서브 공은 x=56(또는 376), y=0, xVelocity=0, yVelocity=1 로 고정 시작한다.
 *    즉 궤적이 완전히 결정적 -> 최적 접촉 시점을 정확히 계산할 수 있다.
 *    y=0 파워히트는 yVelocity를 0으로 만들어 xVelocity ±20 의 수평 직선타가 되고,
 *    접촉 높이가 y≈120 정도면 네트(y=176)를 여유 있게 넘어 상대 코트 깊숙이 꽂힌다.
 *    접촉이 낮으면 네트행이므로 SERVE_FLAT_MAX_Y 아래에서만 수평타, 아니면 아치.
 *
 *  엔진 소스에서 확인한 사실 — 이 봇의 설계 근거
 ...
 *   - ★ 파워히트는 state===1(점프 중)에서만 발동. 지상 파워히트는 존재하지 않는다.
...
var P = {
  ...
  DEADBAND:        11,   // 목표 x와 이만큼 벌어져야 이동
  HOME_FROM_NET:  111,   // 상대 턴일 때 대기 위치
  WALL_MARGIN:    34,   // 코트 끝에서 남길 여유 (플레이어 반폭이 32)

  REACH_X:        30,   // 충돌 32 중 실제로 노릴 여유 (예측 오차 마진)
  REACH_Y:        18,

  /* 타격 지점 선호도 (점수가 높은 계획을 고른다) */
  W_HEIGHT:      2.21,  // 높은 곳에서 칠수록 가산 (y가 작을수록)
  W_NET:         0.88,  // 네트에 가까울수록 가산
  W_EARLY:       0.86,  // 빨리 칠수록 가산

  /* 파워히트 타이밍 */
  POWER_LEAD:      8,   // 접촉 몇 프레임 전부터 hit=1 을 눌러둘지
  JUMP_MAX_DELAY: 20,   // 몇 프레임 뒤 점프까지 계획에 넣을지

  /* 각도 결정 (파워히트 순간의 y 입력) */
  FLAT_MAX_Y:    118,   // 접촉 높이가 이보다 위(y<값)일 때만 y=0 수평강타 허용
  SMASH_MAX_Y:   122,   // 이보다 더 위 + 네트 근처면 y=1 내리꽂기
  SMASH_NET_DIST: 40,
  OPP_NEAR_NET:   199,   // 상대가 네트에 붙어 있으면 내리꽂기 대신 아치로 뒤를 노림

  /* 서브 전용 — 궤적이 결정적이므로 별도 처리 */
  SERVE_MAX_FRAME: 14,  // 랠리 시작 후 이 프레임 안쪽이면 서브 상황
  SERVE_FLAT_MAX_Y:135, // 서브 접촉이 이보다 위(y<값)면 y=0 수평 스파이크
  SERVE_W_HEIGHT:  5.13, // 서브 때는 높은 접촉을 훨씬 강하게 선호
  SERVE_COMMIT:    29,  // 서브는 접촉까지 멀어도 점프 계획을 실행
...
/* ---------- 8. 파워히트 각도 ---------- */
function pickAngle(s, plan, netDistAtHit, oppNetDist, urgent, isServe) {
  /* 서브: 공이 x속도 0으로 수직 낙하만 하므로 궤적이 완전히 결정적이다.
     충분히 높은 지점에서 잡으면 y=0 수평 강타 -> yVelocity=0, xVelocity=±20 의
     직선 스파이크가 나간다. 낮게 잡히면 네트에 박히므로 아치로 안전하게. */
  if (isServe) { return (plan.y < P.SERVE_FLAT_MAX_Y) ? 0 : -1; }
    ...
    if (st === 1) {
      if (plan.f <= P.POWER_LEAD) {
        ...
        out.y = pickAngle(s, plan, Math.abs(plan.x - NET_X),
                          Math.abs(s.opp.x - NET_X), urgent, isServe);
        G.planHits++;
      }
      return out;
    }
